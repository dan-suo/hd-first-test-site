<template>
  <v-container>
    ...
    <v-row justify="center">
      <v-col cols="12" md="6">
        <v-card-title>Введите тут свою дату рождения</v-card-title>
        <v-text-field
          v-model="inputDate"
          label="Введите дату рождения (UTC)"
          type="datetime-local"
          outlined
          dense
        ></v-text-field>
      ...
      <v-col cols="12" md="4" class="d-flex align-center">
        <v-btn color="primary" @click="calculateDesign">Рассчитать</v-btn>
      </v-col>
    </v-row>

    <!-- Чёрный расчёт -->
    <v-row justify="center" v-if="blackData.length">
      <v-col cols="12" md="8">
        <v-card>
    ...
    <!-- Красный расчёт -->
    <v-row justify="center" v-if="redData.length" class="mt-6">
      <v-col cols="12" md="8">
...
const calculateDesign = async () => {
  ...
  blackData.value = [...black]; // Принудительно обновляем массив
  redData.value = [...red];

  await nextTick(); // Ждём, пока Vue увидит изменения
...
/*const resetElementsColor = () => {
  document.querySelectorAll("[id^='gate-'], [id^='chanel-']").forEach((el) => {
    el.style.fill = "white";
  });
};*/
...
/*const colorizeElement = () => {
  ...
  if (el) {
    ...
    console.error("❌ Ошибка: элемент не найден!");
  }
};*/

const blackGates = ref([]); // Чёрный (сознательный) дизайн
const redGates = ref([]); // Красный (несознательный) дизайн
  ...
  document
    ...
    .forEach((el) => {
      ...
      if (
        ...
        el.removeAttribute("fill");
        el.style.fill = "url(#black-red-gradient)"; // Половина чёрный, половина красный
