  ...
  <v-container class="container-of-content mt-7">
    ...
    <v-row justify="center" class="mt-n10">
      ...
      <v-col cols="12" md="2" class="d-flex align-center">
        <v-btn color="primary" size="large" @click="calculateDesign"
          >Рассчитать</v-btn
        >
      </v-col>
    </v-row>
    ...
    <!-- Чёрный расчёт -->
    <v-row justify="center" v-if="blackData.length && tablesVisibility">
      <v-col cols="12" md="8">
        <v-card>
          <v-card-title class="text-h5"
            >Чёрный расчёт (на момент рождения)</v-card-title
          >
          <v-card-text>
            <v-table>
              <thead>
              ...
              <tbody>
                <tr v-for="(item, index) in blackData" :key="'black-' + index">
                  <td>{{ item.planet }}</td>
                  <td>{{ item.degree }}</td>
                  <td>{{ item.gate }}</td>
                  <td>{{ item.line }}</td>
    ...
    <v-row
      justify="center"
      v-if="redData.length && tablesVisibility"
      class="mt-6"
    >
      <v-col cols="12" md="8">
        <v-card color="deep-orange-lighten-5">
          <v-card-title class="text-h5"
            >Красный расчёт (за 88 дней до рождения)</v-card-title
          >
          <v-card-text>
            <v-table>
              <thead>
              ...
              <tbody>
                <tr v-for="(item, index) in redData" :key="'red-' + index">
                  <td>{{ item.planet }}</td>
                  <td>{{ item.degree }}</td>
                  <td>{{ item.gate }}</td>
                  <td>{{ item.line }}</td>
...
/* Указываем порядок планет для того, чтобы отсротировать полученный массив */
const planetOrder = [
  "Sun",
  "Earth",
  "Rahu",
...
const redGates = ref([]); // Красный (несознательный)

const updateGates = () => {
  if (!blackData.value.length || !redData.value.length) {
    console.warn("❌ Ошибка: данные для окраски SVG не получены!");
    return;
  }

  blackGates.value = blackData.value.map((entry) => String(entry.gate));
...
const updateSvgColors = () => {
  ...
  document
    ...
    .forEach((el) => {
      ...
      if (
        blackGates.value.includes(gateNumber) &&
        redGates.value.includes(gateNumber)
      ) {
        el.removeAttribute("fill");
        el.style.fill = "url(#black-red-gradient)";
      } else if (blackGates.value.includes(gateNumber)) {
        el.removeAttribute("fill");
        el.style.fill = "black";
      } else if (redGates.value.includes(gateNumber)) {
        el.removeAttribute("fill");
        el.style.fill = "red";
      } else {
        el.removeAttribute("fill");
  ...
  document.querySelectorAll("[id^='gate-text-']").forEach((el) => {
    const gateNumber = el.id.replace("gate-text-", ""); // Получаем номер ворот
...
const getDefinedCenters = () => {
  ...
  Object.entries(centerChannels).forEach(([center, channels]) => {
    const isDefined = channels.some(({ gate1, gate2 }) => {
      return (
        (blackGates.value.includes(gate1.toString()) ||
          redGates.value.includes(gate1.toString())) &&
        (blackGates.value.includes(gate2.toString()) ||
    ...
    activeCenters[center] = isDefined;
  });

  return activeCenters;
};

// Закрашиваем центры, если есть хоть один закрашенный канал
const updateCenterColors = () => {
  const definedCenters = getDefinedCenters();

  Object.entries(definedCenters).forEach(([center, isDefined]) => {
    const el = document.getElementById(center);
    if (el) {
      el.style.fill = isDefined ? centerColors[center] : "white";
    }
  });
};
